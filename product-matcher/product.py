...
def prepare_product(product):
    ...
    if len(product.get("choice2s", [])) > 0:
        for choice2s in product.get("choice2s"):
            choice1 = filter(
                lambda item: item.get("Id") == choice2s.get("Choice1Id"),
                product.get("choice1s"),
            )
            ...
            if choice1:
                ...
                additional_data = {
                    ...
                    "choice2id": choice2s["Id"],
                    "code": code,
                    "price": price,
                }
                option1.append(
                    {"name": name, "price": price, "additional_data": additional_data}
                )
    else:
